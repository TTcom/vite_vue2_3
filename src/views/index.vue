<template>
  <div class="app-container index_ahome">
    ...
    <div class="project_content bg_w">
       <div class="pc_box">
          <div class="pc_box_title">项目</div>
          <div class="pc_box_mid">
           <img src="./image/projectmsg.png" >
           <span class="tc_num_text">{{pageData.projectNum}}</span>
          </div>
          <div class="pc_box_bottom">最新：{{pageData.projectName}}</div>
       </div>
       <div class="pc_box">
          <div class="pc_box_title">单位</div>
          <div class="pc_box_mid">
           <img src="./image/unit.png" >
           <span class="tc_num_text">{{pageData.companyNum}}</span>
          </div>
          <div class="pc_box_bottom">最新：{{pageData.companyName}}</div>
       </div>
       <div class="pc_box">
          <div class="pc_box_title">班组</div>
          <div class="pc_box_mid">
           <img src="./image/teams.png" >
           <span class="tc_num_text">{{pageData.classNum}}</span>
          </div>
          <div class="pc_box_bottom">最新：{{pageData.className}}</div>
       </div>
    </div>
    <div class="ptitle pl_40 bg_w">
         <img src="./image/work_check.png" >
         <span>出勤打卡</span>
    </div>
    <div class="pbottom bg_w mb_22">
      <div class="pbottom_son" v-for="(item,i) in workerCheckData" :key="i" :class="{'noborder':i==3}" >
       <div class="mt_29">{{item.name}}</div> 
       <div class="nowrap">
        <span class="big_num tc_num_text">{{item.num}}</span> 
        </div> 
...
export default {
  ...
  data() {
    return {
      ...
      workerCheckData:[
           {name:"今日打卡出勤/人",num:0,url:""},
           {name:"昨日打卡出勤/人",num:0,url:""},
           {name:"本周打卡出勤/人",num:0,url:""},
           {name:"本月打卡出勤/人",num:0,url:""},
      ]
    };
  },
  created(){
       getIndex().then(res=>{
         ...
         this.workerCheckData=[
           {name:"今日打卡出勤/人",num:res.data.todayAttendance,url:""},
           {name:"昨日打卡出勤/人",num:res.data.yesterdayAttendance,url:""},
           {name:"本周打卡出勤/人",num:res.data.weekAttendance,url:""},
           {name:"本月打卡出勤/人",num:res.data.monthAttendance,url:""},
         ]
       })
  },
  methods: {
